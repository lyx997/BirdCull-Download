...
请根据自己的显卡选择安装包，避免把新旧 CUDA 运行库混用：

| 版本 | 推荐使用场景 | GPU 加速支持 |
| --- | --- | --- |
| `v1.0.4` | 最新版，推荐大多数新电脑和 Apple silicon Mac 使用 | Windows NVIDIA RTX / GTX 16、Windows AMD / CPU ONNX 回退、macOS Apple silicon |
| `v1.0.3` | Windows 硬件支持范围更广的版本 | `NVIDIA RTX 20/30/40/50`、`GTX 16`、AMD DirectML、ONNX CPU |
| `v1.0.2` | Windows CUDA 兼容版本 | `NVIDIA RTX 20/30/40/50` 系列、`GTX 16` 系列 |
| `v1.0.1` | 旧显卡兼容版 | 部分较旧 `GTX` 显卡，例如 `GTX 900`、`GTX 10` 系列 |
...
如果显卡不在支持范围内，BirdCull 可能会自动回退到 CPU 模式。
